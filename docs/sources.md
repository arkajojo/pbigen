...
```bash
pbigen sources                                  # list every kind
pbigen test --source <kind> --set key=value ...  # verify connectivity + introspection
```
...
|------|---------|----------------------|---------------|
| `postgres` | `pbigen[postgres]` | `table host port database schema user password` | `PGUSER`, `PGPASSWORD` |
| `redshift` | `pbigen[redshift]` | `table host port database schema user password` | `REDSHIFT_HOST/USER/PASSWORD` |
| `snowflake` | `pbigen[snowflake]` | `table account warehouse database schema user password role` | `SNOWFLAKE_*` |
| `synapse` / `fabric` | `pbigen[synapse]` | `table server database schema user password` | `SYNAPSE_SERVER/DATABASE/USER/PASSWORD` |
| `databricks` | `pbigen[databricks]` | `table host http_path token catalog schema` | `DATABRICKS_HOST/HTTP_PATH/TOKEN` |
| `clickhouse` | `pbigen[clickhouse]` | `table host port database user password secure` | `CLICKHOUSE_HOST/USER/PASSWORD` |
| `athena` | `pbigen[athena]` | `table region database s3_staging_dir workgroup` | `AWS_REGION`, `ATHENA_S3_STAGING_DIR` |
...
pbigen generate --source parquet --set uri=./sales.parquet
pbigen generate --source iceberg --set uri=s3://bucket/warehouse/db/orders
pbigen generate --source delta   --set uri=abfss://data@acct.dfs.core.windows.net/orders
```
...
```bash
pip install "pbigen[cube]"
pbigen generate --source cube \
